...
# ------------------------------------------------------------------------------
# BAM-level reproducibility analysis using only IP samples
# ------------------------------------------------------------------------------
# When peak calling is enabled, an additional correlation analysis and PCA are
# generated after excluding input-control libraries from multiBamSummary.

if calling_peaks == "yes":

    rule plot_correlation_multibam_IP:
        input:
            npz = os.path.join(OUT_ROOT, "multiBamSummary_IP", f"{config['GSE']}_multiBamSummary.IP.npz")
        output:
            heatmap = os.path.join(QC_ROOT, "reproducibility", "IP_only", "correlation", f"{config['GSE']}.multiBamSummary.IP.spearman_heatmap.pdf"),
            matrix = os.path.join(QC_ROOT, "reproducibility", "IP_only", "correlation", f"{config['GSE']}.multiBamSummary.IP.spearman_matrix.tab")
        params:
            job_name = f"{config['GSE']}_MULTIBAM_IP_CORRELATION"
        resources:
            time_min = int(custom_param("plotCorrelation_time", 30)),
            cpus = int(custom_param("plotCorrelation_cpus", 4)),
            mem_gb = int(custom_param("plotCorrelation_mem_gb", 16))
        log:
            os.path.join(LOG_ROOT, "reproducibility", "IP_only", "correlation", f"plotCorrelation.multiBamSummary.IP.{config['GSE']}.log")
        benchmark:
            os.path.join(LOG_ROOT, "reproducibility", "IP_only", "correlation", "benchmark", f"plotCorrelation.multiBamSummary.IP.{config['GSE']}.benchmark")
        conda:
            os.path.join(ENV_DIR, "deeptools.yaml")
        shell:
            r"""
            ...
            mkdir -p {QC_ROOT}/reproducibility/IP_only/correlation \
                     {LOG_ROOT}/reproducibility/IP_only/correlation/benchmark
    ...
    rule plot_pca_multibam_IP:
        input:
            npz = os.path.join(OUT_ROOT, "multiBamSummary_IP", f"{config['GSE']}_multiBamSummary.IP.npz")
        output:
            plot = os.path.join(QC_ROOT, "reproducibility", "IP_only", "PCA", f"{config['GSE']}.multiBamSummary.IP.PCA.pdf"),
            table = os.path.join(QC_ROOT, "reproducibility", "IP_only", "PCA", f"{config['GSE']}.multiBamSummary.IP.PCA.tab")
        params:
            job_name = f"{config['GSE']}_MULTIBAM_IP_PCA"
        resources:
            time_min = int(custom_param("plotPCA_time", 30)),
            cpus = int(custom_param("plotPCA_cpus", 4)),
            mem_gb = int(custom_param("plotPCA_mem_gb", 16))
        log:
            os.path.join(LOG_ROOT, "reproducibility", "IP_only", "PCA", f"plotPCA.multiBamSummary.IP.{config['GSE']}.log")
        benchmark:
            os.path.join(LOG_ROOT, "reproducibility", "IP_only", "PCA", "benchmark", f"plotPCA.multiBamSummary.IP.{config['GSE']}.benchmark")
        conda:
            os.path.join(ENV_DIR, "deeptools.yaml")
        shell:
            r"""
            ...
            mkdir -p {QC_ROOT}/reproducibility/IP_only/PCA \
                     {LOG_ROOT}/reproducibility/IP_only/PCA/benchmark
...
rule multiqc:
    ...
    params:
        ...
        # Ignore unwanted deepTools PCA and correlation tables while scanning all QC files.
        ignore_deeptools = (
            '--ignore "*multiBamSummary*.PCA.tab" '
            '--ignore "*multiBamSummary*.spearman_matrix.tab" '
            '--ignore "*spikein_median.PCA.tab" '
            '--ignore "*spikein_median.spearman_matrix.tab"'
            if spikeIN == "yes"
            else
            '--ignore "*multiBamSummary*.PCA.tab" '
            '--ignore "*multiBamSummary*.spearman_matrix.tab"'
