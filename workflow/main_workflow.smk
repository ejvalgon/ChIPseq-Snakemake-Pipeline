...
if calling_peaks == "yes":
    rule multiBamSummary_IP:
        input:
            expand(os.path.join(MAIN_BAM_DIR, "{sample}.bam"), sample=IP_SAMPLES)
        output:
            npz = os.path.join(OUT_ROOT, "multiBamSummary_IP", f"{config['GSE']}_multiBamSummary.IP.npz"),
            raw_counts = os.path.join(OUT_ROOT, "multiBamSummary_IP", f"{config['GSE']}_multiBamSummary.IP.txt")
        params:
            job_name = f"{config['GSE']}_multiBamSummary_IP"
        resources:
            time_min = int(custom_param("multiBamSummary_time", 60)),
            cpus = int(custom_param("multiBamSummary_cpus", 20)),
            mem_gb = int(custom_param("multiBamSummary_mem_gb", 32))
        log:
            os.path.join(LOG_ROOT, "multiBamSummary_IP", f"multiBamSummary.IP.{config['GSE']}.log")
        benchmark:
            os.path.join(LOG_ROOT, "multiBamSummary_IP", "benchmark", f"multiBamSummary.IP.{config['GSE']}.benchmark")
        conda:
            os.path.join(ENV_DIR, "deeptools.yaml")
        shell:
            """
            mkdir -p {OUT_ROOT}/multiBamSummary_IP \
                     {LOG_ROOT}/multiBamSummary_IP/benchmark
...
if spikeIN == "yes":
    ...
    if calling_peaks == "yes":
        ...
        # Generate CPM-normalized bigWig tracks for input control samples.
        rule bamcoverage_bigwig_input_cpm:
            input:
                bam = os.path.join(OUT_ROOT, "split_bam/main", "{sample}.bam"),
                bai = os.path.join(OUT_ROOT, "split_bam/main", "{sample}.bam.bai")
            output:
                os.path.join(OUT_ROOT, "bigwig_input_cpm", "{sample}.coverage.input_cpm.bw")
            params:
                job_name = f"{config['GSE']}_{{sample}}_BAMCOV_INPUT_CPM",
                blacklist = config[ORGANISM]["blacklist_bed"],
            ...
            conda:
                os.path.join(ENV_DIR, "deeptools.yaml")
            log:
                os.path.join(LOG_ROOT, "bigwig_input_cpm", "bamCoverage.{sample}.input_cpm.log")
            benchmark:
                os.path.join(LOG_ROOT, "bigwig_input_cpm", "benchmark", "bamCoverage.{sample}.input_cpm.benchmark")
            shell:
                r"""
                set -euo pipefail

                mkdir -p {OUT_ROOT}/bigwig_input_cpm \
                         {LOG_ROOT}/bigwig_input_cpm/benchmark
...
if spikeIN == "yes":
    rule multiBigwigSummary:
        input:
            median = expand(os.path.join(OUT_ROOT,"bigwig_spikein_median","{sample}.coverage.spikein_median.bw"),sample=IP_SAMPLES),
            deseq2 = expand(os.path.join(OUT_ROOT,"bigwig_spikein_deseq2","{sample}.coverage.spikein_deseq2.bw"),sample=IP_SAMPLES)
        output:
    ...
    rule multiBigwigSummary:
        input:
            expand(os.path.join(OUT_ROOT, "bigwig", "{sample}.coverage.bw"), sample=IP_SAMPLES)
        output:
            npz = os.path.join(
                OUT_ROOT,
                "multiBigwigSummary",
